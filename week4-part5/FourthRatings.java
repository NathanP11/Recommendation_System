...
public class FourthRatings {
    ...
    public boolean checkPopularity ( String IMDB ) {
        Random ran = new Random(); 
        int PopularityTHRESHOLD = 7; //of 150 random
        int ratingCount = 0;
        ArrayList<Rater> raters = RaterDatabase.getRaters();
        for( int i = 0 ; i < 150 ; i++ ){
            Rater r = raters.get(ran.nextInt(raters.size()));
            if( r.hasRating(IMDB) ){
                ratingCount++;
                if( ratingCount == PopularityTHRESHOLD ) return true;
            }
        }
        return false;
    }
